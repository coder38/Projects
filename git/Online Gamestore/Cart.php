...
if(isset($_POST))
{
	...
	else 
	{
		for ($i=0; $i<count($asin_list);$i++)
		{
			...
			$q		 = mysql_result($result2, 0,"quantity");
			$price		 = mysql_result($result2, 0,"price");
			echo "<br>quantity $q";
			echo "<br>qty_list $qty_list";
			$new_q= $qty_list + $q;
			echo "<br>new quantity $new_q <br>";
			
			//if asin exists, update qty
			if ($num2 != 0)
			{
				$query3 = "update cart2 set quantity = $new_q, 
							price=(select price from game2 where asin = '$asin_list[$i]')*$new_q  
							where c_id= $c_id and asin ='$asin_list[$i]'";
				echo $query3;
				mysql_query($query3);
			}
			...
			else 
			{
				...
				//$query = "insert into cart (c_id, asin, quantity) values ($c_id, '$asin_list[$i]', '$qty_list')";
				 ...
				 $query="insert into cart2 values ($c_id,'$asin_list[$i]',
						 ...
						 ((select price from game2 where asin = '$asin_list[$i]')*$qty_list))";
